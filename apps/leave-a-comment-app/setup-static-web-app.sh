#!/bin/bash
# LeaveACommentApp - Azure Static Web App Setup Script
...
# Configuration
APP_NAME="leave-a-comment-app"
RESOURCE_GROUP="${RESOURCE_GROUP:-$APP_NAME}"  # Default to dedicated RG per POC
LOCATION="centralus"
STATIC_WEB_APP_NAME="${APP_NAME}-web"
SKU="Free"

# GitHub repo settings - REQUIRED for GitHub Actions deployment
GITHUB_REPO="https://github.com/idusortus/azure-planner"
GITHUB_BRANCH="main"
GITHUB_TOKEN="${GITHUB_TOKEN:-}"  # Set via: export GITHUB_TOKEN=$(gh auth token)

echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}TODO App - Static Web App Setup${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
...
SWA_ID=$(echo "$SWA_DETAILS" | grep -o '"id": "[^"]*"' | head -1 | cut -d'"' -f4)
...
cat > "$CONFIG_FILE" <<EOF
{
  ...
  "deployment": {
    "recommended": "Azure Developer CLI (azd up)",
    "alternative": "Azure Static Web Apps CLI (swa)",
    "githubActions": {
...
# API Configuration (update after creating Aspire solution)
API_PORT=7001
...
# Update SETUP_NOTES.md if it exists
...
echo ""
echo -e "${BLUE}1. Create Aspire solution in TestPOCApp/ folder:${NC}"
echo -e "   dotnet new aspire -n TodoApp"
echo -e "   dotnet new webapi -n TodoApp.Api -o src/TodoApp.Api"
echo -e "   dotnet new web -n TodoApp.Web -o src/TodoApp.Web"
echo ""
echo -e "${BLUE}2. Deploy with Azure Developer CLI:${NC}"
echo -e "   cd TestPOCApp/TodoApp"
echo -e "   azd init"
echo -e "   azd up"
echo ""
echo -e "${YELLOW}View your app (once deployed):${NC}"
echo -e "  ${GREEN}https://${SWA_URL}${NC}"
echo ""
echo -e "${YELLOW}Cleanup (when done with POC):${NC}"
echo -e "  Delete entire resource group:"
