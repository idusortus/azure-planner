#!/bin/bash
# Friends Prediction - Azure Database Setup Script
...
LOCATION="centralus"
SQL_SERVER_NAME="${SQL_SERVER_NAME:-}"  # Will prompt if not set
DB_NAME="${APP_NAME}-db"
...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}Friends Prediction - Database Setup${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
...
# Get SQL Server name if not provided
if [ -z "$SQL_SERVER_NAME" ]; then
    echo -e "${YELLOW}Enter SQL Server name (e.g., shared-poc-sql or your existing server):${NC}"
    read -r SQL_SERVER_NAME
    
    if [ -z "$SQL_SERVER_NAME" ]; then
        echo -e "${RED}❌ SQL Server name is required${NC}"
        exit 1
    fi
fi
...
echo -e "  2. Add connection string to your Aspire application"
echo -e "  3. Run EF Core migrations:"
